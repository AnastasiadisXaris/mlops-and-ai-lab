...
def generate_user_item(output_dir: Path) -> None:
    ...
    df = pd.DataFrame(
        {
            "user_id": [
                f"U{np.random.randint(0, n_users):04d}" for _ in range(n_interactions)
            ],
            "item_id": [
                f"I{np.random.randint(0, n_items):04d}" for _ in range(n_interactions)
            ],
            ...
            "timestamp": np.sort(
                np.random.randint(1700000000, 1710000000, n_interactions)
            ),
        }
    )
    before = len(df)
    ...
    out = (
        output_dir / "recommendation" / "recommendation-synthetic-user-item-v1.parquet"
    )
    out.parent.mkdir(parents=True, exist_ok=True)
    df.to_parquet(out, index=False)
